...
type PronounceTextButtonProps = {
  text: string;
  sourceLanguage?: string | null;
  className?: string;
  allowEnglishAccentControls: boolean;
};

export function PronounceTextButton({
  text,
  sourceLanguage,
  className,
  allowEnglishAccentControls,
}: PronounceTextButtonProps) {
  ...
  const normalizedLanguage = normalizeSpeechLang(sourceLanguage);
  const usesEnglishAccents =
    allowEnglishAccentControls &&
    shouldUseEnglishAccentControls(sourceLanguage);
  ...
  useEffect(() => {
    ...
    if (!usesEnglishAccents) {
      setVoiceAvailability({ us: false, uk: false });
      return;
    }
  ...
  function handleSpeak(event: MouseEvent<HTMLButtonElement>) {
    ...
    if (usesEnglishAccents) {
      ...
      speakText(text, { lang: normalizedLanguage });
    } else {
      speakText(text);
    }
  }
  ...
  const hasBothAccents =
    usesEnglishAccents && voiceAvailability.us && voiceAvailability.uk;
